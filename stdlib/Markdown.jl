...
renderMD(h::Markdown.Header{l}) where l =
  Container{Symbol(:h, l)}(Attrs(), map(renderMDinline, flatvec(h.text)))
...
renderMD(md::Markdown.Admonition) =
  @dom[:div class="admonition $(md.category)"
    [:p class="admonition-title" md.title]
    renderMD(md.content)]

renderMD(md::Markdown.List) =
  Container{Markdown.isordered(md) ? :ol : :ul}(
    Attrs(:start=>md.ordered > 1 ? string(md.ordered) : ""),
...
renderMDinline(code::Markdown.Code) =
  @dom[:code class="inline" code.code]
